...
import { ArrowUp, CheckCircle, Menu, X } from 'lucide-react';
import type {
  BiographyCategory,
  EditableImageTarget,
  EditableTemplateSection,
} from '../LifeJourney/types';
import { adaptBiographyToEntrepreneur } from './entrepreneurAdapter';
import { biographyData as defaultEntrepreneurData } from './src/data';
import { themeStylesMap } from './src/theme';
import type { BiographyData, TemplateStyle } from './src/types';

// Import our modular subcomponents
import HeroSection from './src/components/HeroSection';
import AboutSection, { PursuitsSection } from './src/components/AboutSection';
...
const isEntrepreneurBiographyData = (data?: BiographyCategory | BiographyData): data is BiographyData =>
  Boolean(data && 'name' in data && 'biographySummary' in data);

const normalizeEntrepreneurData = (data?: BiographyCategory | BiographyData): BiographyData => {
  if (!data) {
    return defaultEntrepreneurData;
  }

  return isEntrepreneurBiographyData(data) ? data : adaptBiographyToEntrepreneur(data);
};

export default function EntrepreneurTemplate({
  data,
  style = 'heritage',
  ...
  const currentStyle: TemplateStyle = style;
  const templateData = normalizeEntrepreneurData(data);
  ...
  const renderTemplateSection = (
    ...
    <div
      id={`${section}-section`}
      className={getEditHighlightClass(section)}
      {...getSectionInteractionProps(section)}
    >
      {children}
    </div>
  );
      ...
      <footer className={`border-t transition-colors duration-500 py-8 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-[11px] font-mono text-stone-500">
            <div>
              &copy; {new Date().getFullYear()} {templateData.name}. All rights reserved. 
              <span className="mx-2">|</span>
              Preserved by Xinghuoji.
            </div>
